...
<div style="display:flex;align-items:center;gap:1rem;margin-bottom:2rem;">
    ...
    <div>
        <h1 style="font-size:1.3rem;font-weight:900;color:var(--navy);">Editar producto</h1>
        <p style="font-size:.8rem;color:var(--gray-400);margin-top:.1rem;">{{ $producto->nombre }}</p>
    </div>
</div>
...
<form method="POST" action="{{ route('admin.productos.update', $producto) }}" enctype="multipart/form-data" id="edit-form">
    @csrf
    @method('PUT')
    ...
    <div style="display:grid;gap:1.5rem;">
            ...
            <div class="admin-card-body">
                <div class="form-group">
                    ...
                    <input type="text" id="nombre" name="nombre" class="form-ctrl"
                           value="{{ old('nombre', $producto->nombre) }}" required maxlength="200">
                    @error('nombre')<div class="form-error">{{ $message }}</div>@enderror
                </div>

                <div class="form-group">
                    <label class="form-label" for="descripcion">Descripción</label>
                    <textarea id="descripcion" name="descripcion" class="form-ctrl" rows="3">{{ old('descripcion', $producto->descripcion) }}</textarea>
                ...
                <div class="form-grid-2">
                    <div class="form-group">
                        ...
                        <select id="categoria" name="categoria" class="form-ctrl" required>
                            ...
                            <option value="{{ $val }}" {{ old('categoria', $producto->categoria) === $val ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="precio_desde" name="precio_desde" class="form-ctrl"
                               ...
                               value="{{ old('precio_desde', $producto->precio_desde) }}" maxlength="50">
        ...
        <div class="admin-card">
            ...
            <div class="admin-card-body">
                ...
                <div style="margin-bottom:1rem;">
                    <div class="form-label" style="margin-bottom:.5rem;">Imágenes actuales (marca las que quieras eliminar)</div>
                    <div style="display:flex;flex-wrap:wrap;gap:.75rem;">
                        @foreach($producto->imagenes as $idx => $imgPath)
                        <div style="position:relative;width:100px;height:100px;border-radius:8px;overflow:hidden;border:2px solid {{ $idx === 0 ? 'var(--coral)' : 'var(--gray-200)' }};">
                            <img src="{{ asset('storage/'.$imgPath) }}" alt="" style="width:100%;height:100%;object-fit:cover;">
                            @if($idx === 0)
                            <div style="position:absolute;bottom:0;left:0;right:0;background:var(--coral);color:white;font-size:.55rem;font-weight:800;text-align:center;padding:.15rem;">Principal</div>
                            @endif
                            <label style="position:absolute;top:4px;right:4px;background:rgba(255,255,255,.9);border-radius:4px;padding:2px 4px;cursor:pointer;font-size:.65rem;font-weight:700;color:#991B1B;">
                                <input type="checkbox" name="imagenes_eliminar[]" value="{{ $imgPath }}" style="margin-right:2px;">✕
                            </label>
                        </div>
                        @endforeach
                    </div>
                ...
                {{-- New images --}}
                <div class="form-group">
                    <label class="form-label">Añadir más imágenes (jpg, png, webp — máx 5 MB cada una)</label>
        ...
        <div class="admin-card">
            ...
            <div class="admin-card-body">
                <p style="font-size:.8rem;color:var(--gray-400);margin-bottom:1rem;">
                    Los colores en los que está disponible la prenda. Los usuarios verán una muestra junto al producto.
                </p>
            ...
            <div class="admin-card-body">
                <div class="form-group" style="max-width:120px;">
                    ...
                    <input type="text" id="emoji" name="emoji" class="form-ctrl"
                           value="{{ old('emoji', $producto->emoji) }}" maxlength="10"
                           style="font-size:1.5rem;text-align:center;">
                </div>

                <div class="form-grid-2" style="margin-top:.8rem;">
                    <div class="form-group">
                        <label class="form-label" for="color_inicio">Color fondo (inicio)</label>
                        <div style="display:flex;align-items:center;gap:.6rem;">
                            <input type="color" id="color_inicio" name="color_inicio"
                                   value="{{ old('color_inicio', $producto->color_inicio ?: '#FF5733') }}"
                                   style="width:44px;height:36px;border:1px solid var(--gray-200);border-radius:6px;cursor:pointer;padding:2px;">
                            <input type="text" id="color_inicio_text"
                                   value="{{ old('color_inicio', $producto->color_inicio ?: '#FF5733') }}"
                                   class="form-ctrl" style="font-family:monospace;flex:1;" maxlength="20" readonly>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="form-label" for="color_fin">Color fondo (fin)</label>
                        <div style="display:flex;align-items:center;gap:.6rem;">
                            <input type="color" id="color_fin" name="color_fin"
                                   value="{{ old('color_fin', $producto->color_fin ?: '#FF8C42') }}"
                                   style="width:44px;height:36px;border:1px solid var(--gray-200);border-radius:6px;cursor:pointer;padding:2px;">
                            <input type="text" id="color_fin_text"
                                   value="{{ old('color_fin', $producto->color_fin ?: '#FF8C42') }}"
                                   class="form-ctrl" style="font-family:monospace;flex:1;" maxlength="20" readonly>
                        </div>
                    </div>
                </div>
                <div id="gradient-preview" style="height:48px;border-radius:10px;margin-top:.5rem;background:linear-gradient(135deg,{{ old('color_inicio', $producto->color_inicio ?: '#FF5733') }},{{ old('color_fin', $producto->color_fin ?: '#FF8C42') }});"></div>

                <div class="form-grid-2" style="margin-top:1rem;">
                    <div class="form-group">
                        <label class="form-label" for="badge">Badge / etiqueta</label>
                        <input type="text" id="badge" name="badge" class="form-ctrl"
                               value="{{ old('badge', $producto->badge) }}" maxlength="100">
                    ...
                    <div class="form-group">
                        ...
                        <select id="badge_tipo" name="badge_tipo" class="form-ctrl">
                            ...
                            <option value="{{ $val }}" {{ old('badge_tipo', $producto->badge_tipo) === $val ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
        ...
        <div class="admin-card">
            ...
            <div class="admin-card-body">
                <p style="font-size:.8rem;color:var(--gray-400);margin-bottom:1rem;">Hasta 6 características cortas. Deja vacías las que no necesites.</p>
                @php $caract = old('caracteristicas', $producto->caracteristicas ?? []); @endphp
                <div class="form-grid-2">
                    @for($i = 0; $i < 6; $i++)
                    <div class="form-group" style="margin-bottom:.5rem;">
                        <input type="text" name="caracteristicas[]" class="form-ctrl"
                               placeholder="Ej: 180g algodón"
                               value="{{ $caract[$i] ?? '' }}" maxlength="100">
                    </div>
                    @endfor
                </div>
            </div>
            ...
            <div class="admin-card-body" style="display:flex;flex-direction:column;gap:.8rem;">
                ...
                <div class="form-group" style="max-width:200px;margin-top:.3rem;">
                    ...
                    <input type="number" id="orden" name="orden" class="form-ctrl"
                           value="{{ old('orden', $producto->orden) }}" min="0">
                </div>
            </div>
        </div>
...
(function () {
    ...
    document.getElementById('imgs-input').addEventListener('change', function () {
        ...
        Array.from(this.files).forEach(function (file) {
            ...
            div.style.cssText = 'width:90px;height:90px;border-radius:8px;overflow:hidden;border:2px solid var(--gray-200);';
    ...
    function buildColorRow(hex, nombre, imgUrl, imgActual) {
        hex       = hex       || '#FFFFFF';
        nombre    = nombre    || '';
        imgUrl    = imgUrl    || '';
        imgActual = imgActual || '';
        ...
        /* Hidden field to pass current image path to server */
        var hiddenActual = document.createElement('input');
        hiddenActual.type  = 'hidden';
        hiddenActual.name  = 'color_imagen_actual[' + i + ']';
        hiddenActual.value = imgActual;
        ...
        fileInput.addEventListener('change', function () {
            if (fileInput.files && fileInput.files[0]) {
                ...
                prevImg.style.display = 'block';
                fileLabel.style.background = '#d1fae5';
                hiddenActual.value = ''; /* old image will be replaced */
            }
        });
    ...
    /* Pre-load existing colors */
    @if(old('color_hex'))
    @foreach(old('color_hex', []) as $i => $hex)
    buildColorRow('{{ $hex }}', '{{ old('color_nombre.'.$i, '') }}', '', '{{ old('color_imagen_actual.'.$i, '') }}');
    @endforeach
    @else
    @foreach($producto->colores ?? [] as $c)
    buildColorRow('{{ $c['hex'] }}', '{{ $c['nombre'] }}', '{{ isset($c['imagen']) && $c['imagen'] ? asset('storage/'.$c['imagen']) : '' }}', '{{ $c['imagen'] ?? '' }}');
    @endforeach
    @endif
